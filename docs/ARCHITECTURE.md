...
**Sync Protocol:** Refer to [SYNC_PROTOCOL.md](file:///c:/Office/editor/docs/SYNC_PROTOCOL.md) for update rules.
...
## EDIT PLAN ARCHITECTURE (AI Editor Brain)

### Overview
The EditPlan is the central data structure that enables AI-driven editing with manual override capability.

```
┌─────────────────────────────────────────────────────────────────┐
│                        EDIT PLAN FLOW                            │
│                                                                  │
│  1. USER INPUTS              2. AI ANALYSIS              3. OUTPUT│
│  ┌────────────┐             ┌────────────┐             ┌────────┐│
│  │ Video Clips│──────┐      │ Beat Map   │──────┐      │EditPlan││
│  │ Audio Track│──────┼─────▶│ Clip Analysis│────┼─────▶│ (JSON) ││
│  │ Style Pref │──────┘      │ AI Director│──────┘      └───┬────┘│
│  └────────────┘             └────────────┘                  │     │
│                                                              │     │
│  4. USER EDIT (Optional)    5. RENDER                       ▼     │
│  ┌────────────┐             ┌────────────┐             ┌────────┐│
│  │ Timeline UI│◀────────────│  EditPlan  │─────────────│ Final  ││
│  │ Drag/Drop  │─────────────▶│   JSON    │────────────▶│ Video  ││
│  └────────────┘             └────────────┘             └────────┘│
└─────────────────────────────────────────────────────────────────┘
```
  ...
  "clips": [
    {
      ...
      "effects": [
        {"type": "zoomPunch", "params": {"intensity": 0.15}}
      ]
    }
  ],
  "total_duration": 30.0,
...
### Services
- **AI Director** (`ai_director.py`): Generates EditPlan from inputs
- **Plan Renderer** (`plan_renderer.py`): Executes EditPlan → final video
- **Effects Service** (`effects.py`): Applies visual effects
- **FFmpeg Handler** (`ffmpeg_handler.py`): Transitions & encoding

### Available Effects
| Effect | Description | Parameters |
|--------|-------------|------------|
| slowmo | Slow motion | speed: 0.25-0.75 |
| speedup | Fast motion | speed: 1.5-4.0 |
| speedramp | Speed varies | start_speed, end_speed |
| freeze | Freeze frame | duration |
| zoomPunch | Quick zoom hit | intensity, duration |
| kenburns | Slow zoom/pan | start_scale, end_scale |
| flashWhite | Flash transition | duration, intensity |
| shake | Camera shake | intensity, frequency |
| vignette | Dark edges | intensity |

### Available Transitions
| Transition | Energy | Description |
|------------|--------|-------------|
| fade | low | Simple cross-fade |
| dissolve | low | Soft dissolve blend |
| fadeblack | low | Fade through black |
| fadewhite | high | Flash transition |
| wipeleft/right | medium | Directional wipe |
| slideleft/right | high | Slide in |
| circlecrop | high | Circular zoom |
| pixelize | high | Pixelation |
